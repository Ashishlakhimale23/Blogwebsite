...
function Card({BlogLink,banner,title,author,publishedOn,_id,bookmarks,handleremovebookmark,handlesavebookmark}:CardType) {
  ...
  return (
    ...
    >
      <button
        ...
        onClick={(e) => {
          e.stopPropagation();
         Array.isArray(bookmarks) && bookmarks.includes(_id) ? handleremovebookmark(_id) : handlesavebookmark(_id);
        }}
        aria-label={Array.isArray(bookmarks) && bookmarks.includes(_id) ? "Remove from bookmarks" : "Save to bookmarks"}
      >
        {Array.isArray(bookmarks) && bookmarks.includes(_id) ? (
