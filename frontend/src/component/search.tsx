...
import { UserBlog, UserPredicatedBlogs } from "../type/types";

interface SearchProps {
  imagesearch: React.RefObject<HTMLButtonElement>;
}

function Search({ imagesearch }: SearchProps) {
  const [searchvalue, setSearchvalue] = useState("");
  const [activetab, setActivetab] = useState("blogs");
  const [blogs,setBlogs] = useState([])
  const [users,setUsers] = useState([])
  const [blogprediction, setBlogprediction] = useState<UserBlog[]>([]);
  const [userprediction, setUserprediction] = useState<UserPredicatedBlogs[]>([]);
...
const debouncedSearch = useCallback(
  debounce((searchTerm:string) => {
    if (searchTerm === "") {
      ...
      (obj:{title:string}) => obj.title.toLowerCase().includes(searchTerm.toLowerCase())
    );
    setBlogprediction(predicatedblogarray);

     const predicateduserarray = users.filter(
      (obj:{username:string}) => obj.username.toLowerCase().includes(searchTerm.toLowerCase()) 
    );
...
useEffect(() => {
 ...
 const handleClickOutside = (event:MouseEvent) => {
    if (popoverRef.current && !popoverRef.current.contains(event.target as Node) && imagesearch.current && !imagesearch.current.contains(event.target as Node) && setSearch) {
      setSearch(false);
    }
  };
