...
function Header() {
  ...
  const { username, pfplink} = info || { username: '', pfplink: '' };
  const popover = useRef<HTMLDivElement>(null);
  const image = useRef<HTMLButtonElement>(null);
  const imagesearch = useRef<HTMLButtonElement>(null);
  const handleClickOutside = (event: MouseEvent) => {
    if (
      popover.current &&
      !popover.current.contains(event.target as Node) &&
      image.current &&
      !image.current.contains(event.target as Node)
    ) {
      setOpen(false);
    }
  };
