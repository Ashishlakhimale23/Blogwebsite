...
function CreatePost() {
  ...
  const { title, content, banner, _id, edited } = blog || { title: "", content: [], banner: "", _id: "", edited: false };
  ...
  useEffect(() => {
    ...
    return () => {
      if (texteditor?.instance && setTexteditor) {
  ...
  const handlebannerinput = async (e:React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (response.ok) {
      ...
      if(BlogbannerRef.current) BlogbannerRef.current.src = data.secure_url;
    } else {
  ...
  const handlepublish = async () => {
    ...
    if (texteditor?.isReady) {
      try {
        ...
        if (savedData.blocks.length > 0 && setBlog) {
