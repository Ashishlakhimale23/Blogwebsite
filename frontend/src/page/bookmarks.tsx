...
import { Bookmarks } from "../type/types";
...
function Bookmark(){
  ...
  const [bookmark, setBookmark] = useState<Bookmarks[]>([DefaultBookmarks]);
  const [loading,setLoading] = useState(true)
  useEffect(() => {
    const fetchbookmarks = async () => {
      const resp : AxiosResponse<Bookmarks[]> = await api.get("/getbookmarks")
 ...
 if(loading){
    return (
      <div className="p-10 animate-shimmer mx-2 bg-gradient-to-r max-w-3xl sm:mx-auto mt-24 rounded-md from-zinc-500/5 via-zinc-500/10 to-zinc-500/5 bg-[length:400%_100%]">
      </div>
    )
  } 
  return (
